...
On staging, every outbound email is a small hazard: it can reach a real
address, and it carries OTPs and magic links. The usual answers are to disable
mail (and lose the ability to test anything that sends it) or to point staging
at a third-party catcher (and hand it your one-time codes).

`stapel-mailtrap` is the third answer: outbound mail is captured into a
`TrappedEmail` journal inside your own database and browsed through a
read-only Mail API. Nothing leaves the deployment, and everything that would
have been sent is inspectable.

It plugs into `stapel-notifications` through that module's email-provider seam
**by dotted path** — one setting, no import between modules, nothing to undo
before production beyond flipping the setting back.
...
Read-only, by design: the trap is filled by the provider or by `trap_email`,
...
The default permission is staff/service only, and that default is deliberate:
trapped mail contains OTPs and magic links, so a wide-open journal is a
credential store with a browsable UI. Override `permission_classes` and set a
`SCOPE_PROVIDER` for per-tenant visibility — see MODULE.md.

## Retention

A trap fills up and nothing ever asks it to stop, so both caps are on by
default:
...
| Kind | Name | Contract |
|---|---|---|
| Event | `mailtrap.email.trapped` | `{email_id, to, from?, subject, scope_key, attachment_count, trapped_at}` — [schema](https://github.com/usestapel/stapel-mailtrap/blob/main/schemas/emits/mailtrap.email.trapped.json) |

## Extension points

See [MODULE.md](https://github.com/usestapel/stapel-mailtrap/blob/main/MODULE.md)
— the agent-facing map of every fork-free seam (settings, the `ScopeProvider`
seam, the notifications email-provider adapter with its duck-typed contract,
serializer seams, permissions, comm surface, system checks).
